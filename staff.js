...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderCalendar(date) {
    ...
    for (let d = 1; d <= lastDay; d++) {
      const dateKey = `${y}-${m + 1}-${d}`;                    // e.g. "2025-7-15"
      const storageKey = `${roleKey}-request-${dateKey}`;
      ...
      const isToday =
        ...
        m === today.getMonth() &&
        y === today.getFullYear();
      const isHoliday = PH_HOLIDAYS.includes(dateKey);
    ...
    // 3d) Wire up clicks to open the modal
